...
<head>
    ...
    <link rel="stylesheet" href="<?= BASEURL ?>css/container.css">
...
<body class="h-svh w-svw bg-50 dark:bg-800 dark:text-50 flex flex-row">
    <div id="overlay" class="bg-white dark:bg-700 dark:text-50 max-[719px]:-left-full min-[720px]:relative min-[720px]:z-100 z-100 min-[720px]:left-0 border-r border-100 dark:border-500 fixed top-0 left-0 flex flex-col justify-between min-w-56 h-full z-50 transition-all ease-linear duration-300">
        <div class="z-10 w-56">
            <div class="p-5 flex items-center gap-3 font-sans font-normal text-md">
                <img src="<?= BASEURL ?>image/logo.png" alt="">
                <a href="tugasupload.html">
                    <div class="font-semibold text-xl text-800 dark:text-white">OUR TASK</div>
                </a>
            </div>
            <button class="w-full  hover:bg-base-100 dark:hover:bg-600 rounded-md" onclick="pathFind('home')">
            ...
            <button class="w-full hover:bg-base-100 dark:hover:bg-600 rounded-md" onclick="pathFind('solo')">
                <div class="flex justify-start items-center p-3 h-8 gap-4 active:translate-y-0.5">
            ...
            <button class="w-full hover:bg-base-100 dark:hover:bg-600 rounded-md" onclick="pathFind('group')">
                <div class="flex justify-start items-center p-3 h-8 gap-4 active:translate-y-0.5">
            ...
            <?php if ($_SESSION['status'] != 'guru') { ?>
                ...
                <button class="w-full relative">
                    <div class="flex justify-start hover:bg-base-100 dark:hover:bg-600 rounded-md items-center p-3 h-8 gap-4" onclick="pathFind('mapel')">
                    ...
                    <div class=" w-full items-start flex flex-col pl-14 hidden delay-1000 transition-all" id="dropdown-mapel">
                        <?php foreach ($data['mapel'] as $mapel) : ?>
                            <a href="<?=BASEURL.'mapel/task/'.$mapel['id_profile']?>" class=""><?= $mapel['mapel'] ?></a>
                        <?php endforeach; ?>
                    </div>
                </button>
            <?php }
            if ($_SESSION['status'] != 'siswa') { ?>
                <button class="w-full hover:bg-base-100 dark:hover:bg-600 rounded-md" onclick="pathFind('home/addtask')">
                    <div class="flex justify-start items-center p-3 h-8 gap-4 active:translate-y-0.5">
        ...
        <div class=" flex flex-col relative justify-center items-start">
            ...
            <form class='w-full' action="<?= BASEURL ?>home/logout" method='post'>
                <button type='submit' class="w-full hover:bg-base-100 dark:hover:bg-600 rounded-md">
                    <div class="flex justify-start items-center p-3 h-8 gap-4 active:translate-y-0.5">
            ...
            <div class="w-full flex items-center mt-5">
                <a href="<?= BASEURL ?>setting/profile">
                    <img src="<?= BASEURL ?>image/Profil.png" alt="" class="flex justify-start items-center p-3 h-14 gap-4 rounded-full">
                </a>
                <div style="font: 400 13px Arial; display: flex; gap: 2px; flex-direction: column;">
                    <div><?= $_SESSION['nama']; ?></div>
                    <div class=" text-xs font-extralight text-300"><?= $_SESSION['email']; ?></div>
                </div>
            </div>
        </div>
    </div>
...
<div class="flex-grow flex flex-col ">
        <!-- Header -->
        <div class=" base-100 bg-white dark:bg-700 relative top-0 right-0 py-3 px-4 border-b border-solid border-100 dark:border-500">

            <div class=" flex justify-between min-[720px]:justify-end items-center h-12 w-full">

                <!-- Humburger -->
                <button class=" border-2 min-[720px]:hidden border-100 dark:border-500 rounded-full size-11 flex justify-center items-center flex-col gap-1 cursor-pointer hover:bg-inherit" id="nav-button" onclick="overlayOpen()">
                    <div class=" h-0.5 w-4 bg-600 dark:bg-gray-600"></div>
                    <div class=" h-0.5 w-4 bg-600 dark:bg-gray-600"></div>
                    <div class=" h-0.5 w-4 bg-600 dark:bg-gray-600"></div>
                </button>
