...
<body class="min-h-svh bg-bg-light dark:bg-item-dark dark:text-slate-200 mx-6 font-sans flex flex-col">
    ...
    <?php foreach ($data['task'] as $task) { ?>
        <h3 class=" font-bold text-3xl mb-4 w-full capitalize">
            <?= $task['name'] ?>
            <p class=" mt-4 text-slate-500 dark:text-gray-300 h-fit w-full font-normal text-base"><?= $task['description'] ?></p>
        </h3>
        <h4 class=" font-extralight text-sm">
            Due date: <?= date('l, j M Y') ?>
        </h4>
        <?php if ($data['task'][0]['id_profile_leader'] != null) { ?>
            <div class="mt-7 text-xl">Member</div>
            <div class="flex items-center flex-wrap">
                <?php
                foreach ($data['member'] as $member) { ?>
                    <img src="<?= BASEURL ?>image/<?= $task['pp_name'] ?>" alt="" class="w-10 h-10 rounded-full m-2.5 justify-center box-border ">
                <?php } ?>
                <?php if ($task['id'] == $task['id_profile'] || sizeof($data['member']) < floor($data['group_lenght'])) { ?>
                    <div id="Add-Member" style="display: none;" class="w-full h-full absolute left-0 top-0">
                        <div class="fixed opacity-40 left-0 top-0 w-full h-full bg-black overflow-auto"></div>
                        <div class="bg-bg-light relative dark:bg-item-dark m-auto z-20 p-5 rounded-lg w-3/5 h-4/5 overflow-hidden pb-4">

                            <!-- Modal content -->
                            <span id="close" class="text-gray-400 float-right text-3xl font-bold cursor-pointer hover:text-gray-500">×</span>
                            <p class="mb-2">Add Member</p>
                            <form action="addMember" method="post" class="h-full w-full relative">
                                <input type="hidden" name="addMember" id="addMemberValue" value="">
                                <div class="h-full pb-20">
                                    <div class="grid h-full overflow-hidden overflow-y-auto">
                                        <?php foreach ($data['notmember'] as $getmember) { ?>
                                            <div style="display: flex;" class="gap-2.5 items-center">
                                ...
                                <button onclick="getValue()" type="submit" class="absolute bottom-0 bg-own-blue text-white h-7 w-full mt-7 rounded-lg" >Add Member</button>
                            </form>
                        </div>
                    </div>
                <?php }
